...
function addCategoryToCategoryList(category) {
  ...
  catLink.addEventListener("click", (e) => {
    e.stopPropagation();
    const index = taskList.categories.findIndex(
...
function editCategoryEventHandler(e) {
  ...
  function saveChangesHandler(event) {
    ...
    const index = linkArr.findIndex(
      (link) => link.textContent.trim() === currentTitleValue,
    );
...
function taskEventListeners() {
  ...
  addTaskBtnDialog.addEventListener("click", (e) => {
    ...
    if (taskTitle.value && taskDueDate.value) {
      ...
      const index = taskList.categories.findIndex(
        (category) => category.name === title.textContent,
      );
...
function addTaskCard(task) {
  ...
  const taskExists = Array.from(existingTasks).some(
    (existingTask) =>
      task.title === existingTask.querySelector(".task-title").textContent,
  );
...
function deleteTaskButtonClickHandler(e) {
  const catName =
    e.target.parentElement.parentElement.parentElement.firstChild.textContent.trim();
  const taskName = e.target.parentElement.firstChild.textContent.trim();
  const catIndex = taskList.categories.findIndex(
    (cat) => cat.name.trim() === catName,
  );
...
function editTaskButtonClickHandler(e) {
  ...
  const renderedTaskTitle =
    e.target.parentElement.firstChild.textContent.trim();
  const renderedTaskDescription =
    e.target.previousElementSibling.previousElementSibling.previousElementSibling.textContent.trim();
  const renderedDueDate =
    e.target.previousElementSibling.previousElementSibling.textContent
      .trim()
      .replace("Due Date: ", "");
  renderedDueDate.replaceAll("-", " ");
  const renderedPriority = e.target.previousElementSibling.textContent
    .trim()
    .replace("Priority: ", "");
  ...
  addTaskBtnDialog.addEventListener("click", (e) => {
    ...
    if (taskTitle.value && taskDueDate.value) {
      ...
      const taskIndex = cat.tasks.findIndex(
        (task) => task.title === renderedTaskTitle,
      );
...
function updateCard(task, oldTitle) {
  ...
  const index = cardArr.findIndex(
    (card) => card.firstChild.textContent.trim() === oldTitle,
  );
...
function taskDoneButtonClickHandler(e) {
  ...
  const taskIndex = cat.tasks.findIndex(
    (task) =>
      task.title === e.target.parentElement.firstChild.textContent.trim(),
  );
  const task = cat.tasks[taskIndex];
  // toggle task status
